...
export default function TaskModal({ open, onClose, task, projects, users, currentUser, onSave, onDelete, defaultProject }: TaskModalProps) {
  ...
  const selectStyle = {
    width: "100%", padding: "10px 12px", border: `1px solid ${C.g200}`,
    borderRadius: 8, fontSize: 13, fontFamily: "inherit", outline: "none",
    background: C.w, color: C.navy,
  };
    ...
    <Modal open={open} onClose={onClose} title={isEdit ? t.tmod_edit : t.tmod_create}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12, marginBottom: 12 }}>
        <div>
          ...
          <select value={status} onChange={(e) => setStatus(e.target.value as Task["status"])} data-testid="task-status-select" style={selectStyle}>
        ...
        <div>
          ...
          <select value={priority} onChange={(e) => setPriority(e.target.value as Task["pri"])} data-testid="task-priority-select" style={selectStyle}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12, marginBottom: 12 }}>
        <div>
          ...
          <input type="date" value={due} onChange={(e) => setDue(e.target.value)} data-testid="task-due-input" style={{ ...selectStyle, boxSizing: "border-box" }} />
        </div>
        <div>
          <label style={{ fontSize: 12, fontWeight: 600, color: C.g600, display: "block", marginBottom: 6 }}>{t.tmod_project}</label>
          <select value={pId} onChange={(e) => setPId(e.target.value)} data-testid="task-project-select" style={selectStyle}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <select value={assignee} onChange={(e) => setAssignee(e.target.value)} data-testid="task-assignee-select" style={selectStyle}>
