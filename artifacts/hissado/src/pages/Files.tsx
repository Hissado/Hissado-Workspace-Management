...
export default function Files({ files, folders, users, onAddFile, onAddFolder }: FilesProps) {
  ...
  // fId in FileItem is the folder reference
  ...
  const createFolder = () => {
    ...
    const f: Folder = {
      id: uid(), name: newFolderName.trim(), pId: firstProjId,
    };
    onAddFolder(f);
    setNewFolderName("");
      ...
      {searchQuery ? (
        <section>
          ...
          {searchResults?.length === 0 ? (
            ...
            <FileGrid files={searchResults || []} userMap={userMap} />
          )}
        </section>
      ) : (
        <>
            ...
            <section style={{ marginBottom: 32 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 12 }}>
                {folders.map((folder) => {
                  ...
                  return (
                    <button key={folder.id} onClick={() => setActiveFolderId(folder.id)} data-testid={`folder-${folder.id}`}
                      style={{ padding: "16px 20px", background: C.w, border: `1px solid ${C.g100}`, borderRadius: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 12, textAlign: "left", transition: "box-shadow .15s", fontFamily: "inherit" }}
                      onMouseEnter={(e) => e.currentTarget.style.boxShadow = "0 4px 16px rgba(0,0,0,.08)"}
                      onMouseLeave={(e) => e.currentTarget.style.boxShadow = "none"}
                    >
                      <div style={{ color: C.gold, flexShrink: 0 }}><FolderIconSvg /></div>
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: C.navy }}>{folder.name}</div>
          ...
          <section>
            ...
            {displayFiles.length === 0 ? (
              ...
              <FileGrid files={displayFiles} userMap={userMap} />
            )}
          </section>
        </>
      )}
...
function FileGrid({ files, userMap }: { files: FileItem[]; userMap: Record<string, User> }) {
  return (
